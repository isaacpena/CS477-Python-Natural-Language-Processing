...
class LogisticRegression:
    ...
    def __init__(self, dimension, num_classes):
        ...
        self.prob_y_given_x = T.nnet.softmax(T.dot(X, self.W) + self.b)
        ...
        self.cost = -T.mean(T.log(self.prob_y_given_x)[T.arange(y.shape[0]), y])

        self.g_W = T.grad(cost=self.cost, wrt=self.W)
        self.g_b = T.grad(cost=self.cost, wrt=self.b)
        
        updates = [(self.W, self.W - learning_rate * self.g_W), (self.b, self.b - learning_rate * self.g_b)]
