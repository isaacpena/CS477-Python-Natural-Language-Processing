...
class DependencyRNN:
    ...
    def __init__(self, d, V, r, answer_idxs, embeddings=None, seed=0):
        ...
        def normalized_tanh(x):
            ...
            return theano.tensor.tanh(x) / theano.tensor.sqrt(theano.tensor.sum(theano.tensor.tanh(x) ** 2))
        ...
        def recurrence(n, hidden_states, hidden_sums, cost, x, r, p, wrong_ans, corr_ans):
            '''
            unction called below by scan over the nodes in the dependency parse
            ...
            (in that order)
            '''
            hn_calc = self.f(T.dot(self.Wv, x[n]) + self.b + hidden_sums[n])
            new_h_state = T.set_subtensor(hidden_states[n], hn_calc)
            new_h_sums = T.set_subtensor(hidden_sums[p[n]], T.dot(r[n], hn_calc) + hidden_sums[p[n]])

            def wrong_ans_calc(m, cost_of_wrong_ans, hn_calc, corr_ans):
                return cost_of_wrong_ans + T.maximum(0, 1 - T.dot(corr_ans, hn_calc) + T.dot(m, hn_calc))

            c, c_updates = theano.scan(fn=wrong_ans_calc,
                                sequences=wrong_ans,
                                outputs_info = T.as_tensor_variable(np.asarray(0, theano.config.floatX)),
                                non_sequences = [hn_calc, corr_ans])     

            new_cost = cost + c[-1]

            return new_h_state, new_h_sums, new_cost
